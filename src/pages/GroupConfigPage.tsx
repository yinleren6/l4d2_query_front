...
export default function GroupConfigPage() {
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) {
        toast.error(err.response?.data?.error || "Token无效或无权限访问该群组配置");
      }
    } finally {
  ...
  return (
    <Card className="p-6 max-w-2xl mx-auto mt-8">
      ...
      <div className="flex gap-2 mb-6">
        <Input value={token} onChange={(e) => setToken(e.target.value)} placeholder="输入该群组的Token" onKeyDown={(e) => e.key === "Enter" && loadConfig()} className="flex-1" disabled={loading || saving} />
      ...
      {config ? (
        <div className="space-y-4">
          <div>
            <label className="text-sm font-medium mb-1 block">群组ID</label>
            <Input value={config.group_id} disabled readOnly />
          </div>

          <div>
            <label className="text-sm font-medium mb-1 block">自动回复</label>
            <select
          ...
          <div>
            <label className="text-sm font-medium mb-1 block">最大消息数</label>
            <Input
              type="number"
          ...
          <Button onClick={saveConfig} disabled={loading || saving} className="w-full sm:w-auto">
            {saving ? "保存中..." : "保存配置"}
          </Button>
        </div>
      ) : (
        <div className="p-4 text-center text-muted-foreground">请输入有效Token加载配置</div>
      )}
    </Card>
  );
}
