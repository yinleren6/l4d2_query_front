    ...
    <Card className="p-6 w-full h-full min-h-[700px] animate-fade-slide">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {list.map((item) => (
            <div key={item.group_id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border rounded-lg gap-3">
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{item.group_id}</span>
                  <span className={`text-xs px-2 py-0.5 rounded ${item.enabled ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>{item.enabled ? "已启用" : "已禁用"}</span>
                </div>
                <p className="text-sm text-slate-500">添加时间: {item.added_at}</p>

                <div className="flex items-center gap-2 mt-1">
                  <div className="flex-1 bg-slate-100 p-2 rounded text-sm font-mono overflow-hidden text-ellipsis">{item.token}</div>
                  <Button variant="ghost" size="icon" onClick={() => copyToken(item.token, item.group_id)} disabled={copyingId === item.group_id}>
                    <Copy size={16} />
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => refreshToken(item.group_id)} disabled={!item.enabled}>
                    <RefreshCw size={16} />
                  </Button>
                </div>

                <div className="flex gap-2 mt-1">
                  <Button variant="secondary" size="sm" onClick={() => editGroupConfig(item.group_id)}>
                    编辑群组配置
                  </Button>
                  {/* 新增删除按钮 */}
                  <Button variant="destructive" size="sm" onClick={() => deleteGroup(item.group_id)}>
              ...
              <Button variant={item.enabled ? "destructive" : "default"} size="sm" onClick={() => toggleEnable(item.group_id, item.enabled)}>
                {item.enabled ? "禁用" : "启用"}
              </Button>
            </div>
          ))}
