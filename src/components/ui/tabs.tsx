...
function Tabs({ className, orientation = "horizontal", ...props }: React.ComponentProps<typeof TabsPrimitive.Root> & { orientation?: "horizontal" | "vertical" }) {
  return (
    <TabsPrimitive.Root
      data-slot="tabs"
      data-orientation={orientation}
      className={cn("flex", orientation === "horizontal" && "flex-col gap-4", orientation === "vertical" && "flex-row gap-4", className)}
      {...props} // orientation 不会传递给原生 Root，但 data- 属性已添加
    />
  );
}
// TabsList导航栏：强制横向排列
const tabsListVariants = cva("inline-flex items-center justify-center rounded-lg p-1 text-muted-foreground", {
  variants: {
    variant: {
      default: "bg-muted",
